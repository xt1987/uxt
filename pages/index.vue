<template>
	<uxt-layout1 :headerHeight="0" :footerHeight="100">
		<view v-if="tabIndex === 0">
			<image src="/static/banner/basics.png" mode="widthFix" class="response"></image>
			<view class="nav-list">
				<navigator hover-class="none" :url="`/pages/basics/${item.name}`" class="nav-li a" navigateTo :class="[`bg-${item.color}`]"
				  :style="[{animationDuration: ((index + 1) * 0.2 + 1) + 's'}]" v-for="(item, index) in basics" :key="item.name">
					<view class="nav-title">{{item.title}}</view>
					<view class="nav-name">{{item.name}}</view>
					<view class="nav-icon">
						<uxt-icon :type="item.icon"></uxt-icon>
					</view>
				</navigator>
			</view>
		</view>
		<view v-else-if="tabIndex === 1">
			<image src="/static/banner/components.png" mode="widthFix" class="response"></image>
			<view class="nav-list">
				<navigator hover-class="none" :url="`/pages/components/${item.name}`" class="nav-li a" navigateTo :class="[`bg-${item.color}`]"
				 :style="[{animationDuration: ((index + 1) * 0.2 + 1) + 's'}]" v-for="(item, index) in components" :key="item.name">
					<view class="nav-title">{{item.title}}</view>
					<view class="nav-name">{{item.name}}</view>
					<view class="nav-icon">
						<uxt-icon :type="item.icon"></uxt-icon>
		...
		<template #footer>
			<uxt-tab-bar :items="navs" @itemClick="itemClick"></uxt-tab-bar>
		</template>
	</uxt-layout1>
</template>
	...
	export default {
		...
		computed: {
			navs() {
				const that = this;
				return [{
					icon: `/static/tabbar/basics${that.tabIndex === 0 ? '_active' : ''}.png`,
					text: '基础',
					color: that.tabIndex === 0 ? that.gTheme : null
				}, {
					icon: `/static/tabbar/component${that.tabIndex === 1 ? '_active' : ''}.png`,
					text: '组件',
					color: that.tabIndex === 1 ? that.gTheme : null
				}, {
					icon: `/static/tabbar/plugin${that.tabIndex === 2 ? '_active' : ''}.png`,
					text: '扩展',
					color: that.tabIndex === 2 ? that.gTheme : null
				}, {
					icon: `/static/tabbar/about${that.tabIndex === 3 ? '_active' : ''}.png`,
					text: '关于',
					color: that.tabIndex === 3 ? that.gTheme : null
				}];
			}
		},
		methods: {
...
<style>
	...
	.a {
		animation: ease 0s 1 normal none running show;
	}
